...
enum NotesReader {
    ...
    static func listFolders() throws -> [NotesFolder] {
        let script = """
    ...
    static func fetchFirstNote(in folder: NotesFolder) throws -> NotePreview {
        ...
        let id = parts[0]
        let title = parts[1]
        let body = parts.dropFirst(2).joined(separator: fieldSeparator)

        return NotePreview(id: id, title: title, body: body)
    }
