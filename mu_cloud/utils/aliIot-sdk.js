...
//构建公共参数
const _buildParams = (config) => {
  if (!config || !config.ai || !config.apiVersion) {
    throw new Error('缺少必要配置: ai/apiVersion')
  }
...
//封装request
const request = (config, params, opts, success, fail, complete) => {
  if (!config || !config.ai || !config.as || !config.endpoint) {
    throw new Error('缺少必要配置: ai/as/endpoint')
  }
  params = formatParams(params)
  params.Action = firstLetterUpper(params.Action)
  var defaultParams = _buildParams(config)
  ...
  const url = method === 'POST' ? `${config.endpoint}/` : `${config.endpoint}/?${canonicalize(normalized)}`
  if (method === 'POST') {
